...
La diversidad étnica se conserva desde **sector**; el visor deberá deshabilitarla en manzana y explicar el motivo. `P11R` contiene seis categorías recodificadas en SECTOR y CANTÓN; no aparece en MANLOC. `P30` aparece en los tres diccionarios, pero pregunta por aportes a seguridad social y no mide cobertura de seguro de salud. Por ello se descarta ese indicador en los tres niveles. Estas decisiones, su disponibilidad por nivel y `min_level` constan en [`indicators.yaml`](../indicators.yaml). Los demás índices requieren pruebas de denominadores en fase 1.
...
El flujo y los permisos se explican en [`docs/data-access.md`](data-access.md). El repositorio público no usa un secret de acceso al almacén privado ni un PAT. El `GITHUB_TOKEN` automático del workflow privado lee `data-raw-v1`; solo su job de publicación escribe en el Release propio. La CI pública lee solo una muestra agregada versionada; el despliegue restaura datos agregados desde el Release público y nunca accede al almacén privado. El [run privado exitoso](https://github.com/diegocevallos-tech/censo-vivo-ecuador-raw/actions/runs/36052695468) restauró y verificó los 12 assets, ejecutó `00_check.py --sample` y el QA completo, pasó el control de publicación y produjo `fase0_qa.json`. El artifact retiene agregados siete días; el mismo archivo se publicó inicialmente en el Release privado `data-derived-v0`. Se descargó localmente y se comprobó su SHA256 `b283c4fcee85c686b75daad25950bc2bd23b9596b9ac1d34a6e8c2aa157dbd87`, su esquema agregado (24 filas provinciales) y sus totales. Desde Fase 1A, `fase0_qa.json` se sirve desde el [Release público de derivados](https://github.com/diegocevallos-tech/censo-vivo-ecuador/releases/tag/data-derived-v1a), fuera del historial de git.

Al cierre de la Fase 0, antes de crear la rama de conteos, se auditó `git log --all --stat` y los blobs alcanzables con `git rev-list --objects --all` y `git cat-file -s`: **cero archivos `.csv`, `.sav`, `.zip` o `.parquet` mayores de 1 MiB** en aquel historial público. Los únicos dos blobs de esas extensiones eran los CSV de QA de 2.077 y 58.458 bytes. La Fase 1A añade Parquet **agregados** mayores de 1 MiB, descritos en el [informe de conteos](fase1a_report.md); esa incorporación no altera el resultado histórico de este control sobre microdatos. `data/raw/` y `data/interim/` permanecen ignorados. [`pipeline/verify_public_artifacts.py`](../pipeline/verify_public_artifacts.py) rechaza archivos crudos, columnas de registro y assets de 100 MB o más. Los Releases no se usan en el navegador: una prueba `GET` con `Origin` y rango sobre un asset público recibió 302/206 sin `Access-Control-Allow-Origin`; además, el almacén original es privado.

**Resultado del filtro de historial al cierre de Fase 0:** `git log --all --stat -- '*.csv' '*.sav' '*.zip' '*.parquet'`, seguido de `git rev-list --objects --all` y `git cat-file -s` para restringir a blobs mayores de 1 MiB: **sin coincidencias**. Se revisaron también los enlaces de `docs/` y los comentarios de los PR #41 y #1; no contienen rutas locales de Windows. Los enlaces internos de este informe son relativos al repositorio.
