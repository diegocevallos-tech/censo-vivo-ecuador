...
6. Release público publicado con cuatro tar.gz. Se descargó de nuevo y se restauraron 341 archivos con SHA256 coincidente; la QA aditiva completa, incluidas categorías y 96 chunks de conteo/análisis, pasó. El primer [deploy](https://github.com/diegocevallos-tech/censo-vivo-ecuador/actions/runs/36183858675) falló porque `verify_public_artifacts.py` importaba DuckDB en un runner sin ese paquete; el verificador ahora extrae el esquema desde el AST sin dependencias adicionales. Falta reintentar.

## Siguiente comando exacto

```sh
gh workflow run deploy.yml -R diegocevallos-tech/censo-vivo-ecuador --ref feat/fase-1b3-tiles-paquete-web -f preview=false
```

Antes, hacer commit y push del verificador corregido y la QA de chunks ampliada. Luego despachar `deploy.yml` con `preview=false`, comprobar Pages y abrir PR 1B-3. Mantener el PR abierto para revisión. Después seguir 2A y 2B según la autorización vigente sin tocar la auditoría independiente.
